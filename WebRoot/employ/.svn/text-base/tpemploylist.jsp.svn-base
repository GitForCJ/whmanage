...
<%
  String path = request.getContextPath();
   ...
   request.setAttribute("groupsList",ccc.getStingSel(tpbean.getGroups(),"佣金组"));//获取佣金组名称
  ...
  //request.setAttribute("interName",ccc.getStingSel(biProd.listagentsign(),"接口商"));
%>
<html:html>
<head>
<title>万汇通</title>
	...
	<div class="topCtiy clear">
		<ul>
		  ...
		  <li>
		   ...
		   <li>
	  		<div class="sub-input"><a class="sia-2 selhover" id="sl-groups" href="javascript:void(0);"  style="width:170px;">佣金组</a></div>
		  ...
		  <li><input type="button" name="Button" value="查询" class="field-item_button" onClick="checksubmit(0);"></li>
		   <c:if test="${userForm.groups!='' && userForm.groups!=null}">
		  	<li><input type="button" name="Button" value="导出" class="field-item_button" onClick="checksubmit(-11);" id="excelID"></li>
		   </c:if>
		</ul>
	</div>
</div>
