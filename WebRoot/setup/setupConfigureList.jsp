...
<html>
  <head>
    ...
    <script type="text/javascript">
		...
		function funDel(groupsID,userno)
		{
			if(confirm("你确定删除吗?"))
			{
				window.location.href="<%=path %>/business/prod.do?method=configureList&zName=${userForm.zName}&curPage=${curPage}&type=-100&userno="+userno+"&groupsID="+groupsID;
			}
		}
    </script>
  </head>
  <body>
  <form action="<%=path%>/business/prod.do?method=configureList" id="for" method="post">
  	...
  	<table border="0" style="width:100%;font-size:12px;border:1px solid #cccccc;" cellspacing="0" cellpadding="0">
  		<tr style="height:30px;background-color:#E6EFF6;">
  			...
  			<th  style="text-align:center;border-bottom:1px solid #cccccc;border-right:1px solid #cccccc;">系统编号</th>
  			<th style="text-align:center;border-bottom:1px solid #cccccc;border-right:1px solid #cccccc;">账户名</th>
  			<th style="text-align:center;border-bottom:1px solid #cccccc;border-right:1px solid #cccccc;">用户名称</th>
  		...
  		<c:forEach items="${userList}" var="list" varStatus="index">
  			<tr>
  				...
  				<td style="text-align:center;height:30px;border-bottom:1px solid #cccccc;border-right:1px solid #cccccc;">${list[0]}</td>
  				<td style="text-align:center;height:30px;border-bottom:1px solid #cccccc;border-right:1px solid #cccccc;">${list[1]}</td>
  				<td style="text-align:center;height:30px;border-bottom:1px solid #cccccc;border-right:1px solid #cccccc;">${list[2]}</td>
  				...
  				<td style="text-align:center;height:30px;border-bottom:1px solid #cccccc;border-right:1px solid #cccccc;">
  					<a href="javascript:funDel(${list[4]},'${list[1]}')" style="font-size:13px;">删除</a>
  				</td>
  			</tr>
  		</c:forEach>
			...
			<tr >
				<td colspan="7" >
					<div class="grayr" style="text-align:right;height:35px;line-height:35px;font-size:12px;">
					 <wlt:pager url="business/prod.do?method=configureList&zName=${userForm.zName}" page="${requestScope.page}" style="black" />
					</div>
				</td>
			</tr>
		</tfoot>
...
<p align="center">
	<input onClick="javascript:window.location.href='<%=path %>/setup/setupConfigureadd.jsp?zName=${userForm.zName}&curPage=${curPage}'" type="button" style="width:85px; height:32px; border:0px;color:#fff; font-weight:bold;background:url(<%=path%>/images/button_out.png) no-repeat bottom; cursor:pointer;" value="增加"/>
</p>
</form>
  </body>
</html>
