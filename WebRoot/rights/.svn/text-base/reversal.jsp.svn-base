...
<ul style="padding-left:20px;">
	<li>
		...
		<select style="width:120px;height:23px;background-color:white;" name="tradeTypes" id="tradeTypesID">
			...
			<option value="0">冲正</option>
			<option value="1">Q币</option>
			<option value="2">支付宝</option>
			<option value="3">额度转移</option>
            <option value="4">Q币启动多通道</option>
		</select>
		&nbsp;
		&nbsp;
	...
	<label style="margin-left: 5px;color: red;font-size: 110%">当前流量压单状态:[&nbsp;<span id="flow">${flowvalue}</span>&nbsp;]</label> &nbsp;&nbsp;&nbsp;<input onclick="javascript:flow('this');" type="button" style="width:85px;height:32px; border:0px;color:#fff; font-weight:bold;background:url(<%=path%>/images/button_out.png) no-repeat bottom; cursor:pointer;" value="修改"/>
<label style="margin-left:5px;color: red;font-size: 100%">短信通道:</label>		
<select style="width:100px;height:23px;background-color:white;" name="dx" id="duanxin" onchange="javascript:duanxin1(this);">
			<option value="0" <c:if test="${duanxin==0}">selected="selected"</c:if> >烽火科技(旧)</option>
			<option value="1" <c:if test="${duanxin==1}">selected="selected"</c:if> >三三得九</option>
			<option value="2" <c:if test="${duanxin==2}">selected="selected"</c:if> >烽火科技(新)</option>
		</select>
</li>
</ul>
...
<th style="height:25px;border:1px solid #cccccc;">登陆账号</th>
<th style="height:25px;border:1px solid #cccccc;">用户编号</th>
<th style="height:25px;border:1px solid #cccccc;">数量</th>
<th style="height:25px;border:1px solid #cccccc;">类型</th>
...
<p align="center">
  ...
  <input type="button" name="Submit2" value="修改" class="onbutton" onClick="ha_udate()">
</p>
</div>
</html:form>
</body>
