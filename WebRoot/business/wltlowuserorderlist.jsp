...
<style type="text/css">
.tabs_wzdh { width: 120px; float:left;}
...
function checksubmit(){
	...
	$("#form1").first();
	$("#form1").attr("action","<c:url value='/business/order.do?method=listloweruser'/>");
	$("#form1").submit();//提交
	funLoad();
}
function funLoad()
...
function excelExport(){
	...
	$("#form1").first();
	$("#form1").attr("action","<c:url value='/business/order.do?method=loweruserExcelExport'/>");
	$("#form1").submit();//提交
...
<form id="form1" action="<%=path %>/business/order.do?method=listloweruser" method="post">
<div class="header">
	...
	<nobr>
		<ul>
		  ...
		  <div class="sub-input" style="display: none;"><a class="sia-2 selhover" id="sl-term_type" href="javascript:void(0);">终端类别</a></div>
			...
			<li>
		  <div class="sub-input"><a class="sia-2 selhover" id="sl-fee" href="javascript:void(0);">充值金额</a></div>
		  	<input nname="nname" name="fee" id="fee" defaultsel="充值金额[]|10[10]|20[20]|30[30]|50[50]|100[100]|200[200]|300[300]|500[500]" type="hidden" value="${order.fee }" />
		  </li>
			<li>
		  <div class="sub-input"><a class="sia-2 selhover" id="sl-dailishang" href="javascript:void(0);">代理商</a></div>
		  	<input nname="nname" name="dailishang" id="dailishang" defaultsel="${requestScope.dailishangSel }" type="hidden" value="${order.dailishang }" />
		  </li>
		</ul>
...
<td class="td_line">${ordinfo[5]/1000.0}<font color="red">|</font>${ordinfo[6]/1000.0}</td>
<td class="td_line">${ordinfo[7]}</td>

</tr>
</logic:iterate>
<tr><td colspan="8">&nbsp;</td></tr>
<tr><td colspan="8" align="left">当前页交易总额：${requestScope.curTotal/1000.0 }</td></tr>
<tr><td colspan="8" align="left">交易总额:<fmt:formatNumber value="${requestScope.totalFee[0]/1000.0 }" pattern="#0.000"/>&nbsp;交易总面额:<fmt:formatNumber value="${requestScope.totalFee[1]/1000.0 }" pattern="#0.000"/>&nbsp;总笔数:${requestScope.totalFee[2]}</td></tr>
</tbody>
<tfoot>
<tr height="30">
<td colspan="9" class="tr_line_top">
<div class="grayr">
<wlt:pager url="business/order.do?method=listloweruser${requestScope.order.paramUrl}" page="${requestScope.page}" style="black" />
</div>
</td>
<!--
<td colspan="8">
