...
showMessage("${mess}");
	...
	function checksubmit()
	{
		...
		document.getElementById("form1").action="<%=path %>/business/acctbill.do?method=zshCensusList&subtype=0";
	...
	//	alert(ahidden.value.Trim());
		...
		document.getElementById("form1").action="<%=path %>/business/acctbill.do?method=zshCensusList&subtype=1";
		document.getElementById("form1").submit();
		return;
	}
...
<div class="selCity" id="allCity" >
    <table  border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td >
				下级账号:<input type="text" value="${accountName}" id="accountName" name="accountName" style="width:120px;height:25px;" maxlength="11" />
			</td>
		    <td >
...
<div id="tables">
	<table  cellspacing="0" cellpadding="0" style="font-size:13px;border-bottom:1px solid #cccccc;border-right:1px solid #cccccc" >
		...
		<tr style="font-weight:bold;background-color:#E6EFF6;">
			...
			<td style="width:100px;" rowspan=2>笔<br/>数</td>
			<td style="width:100px;" rowspan=2>金额<br/>(元)</td>
			<td style="width:100px;" rowspan=2>笔<br/>数</td>
			<td style="width:100px;" rowspan=2>金额<br/>(元)</td>
			<td style="width:100px;" rowspan=2>笔<br/>数</td>
			<td style="width:100px;" rowspan=2>金额<br/>(元)</td>
		...
		<tr style="background-color:#E6EFF6; font-weight:bold;">
			<td style="width:100px;">笔<br/>数</td>
			<td style="width:100px;">金额<br/>(元)</td>
		...
		<c:forEach items="${accountList}" var="arrList" varStatus="index">
			...
			<c:if test="${(index.index+1)%2!=0}">
				...
				<td style="color:black">${arrList[0]}<br/>${arrList[1]}</td>
				<td >${arrList[2]}&nbsp;</td>
				<td ><fmt:formatNumber value="${arrList[3]}" pattern="#0.0"/>&nbsp;</td>
				<td>${arrList[4]}&nbsp;</td>
				<td><fmt:formatNumber value="${arrList[5]}" pattern="#0.0"/>&nbsp;</td>
				
				<td>${arrList[6]}&nbsp;</td>
				<td ><fmt:formatNumber value="${arrList[7]}" pattern="#0.0"/>&nbsp;</td>
				<td>${arrList[8]}&nbsp;</td>
				<td><fmt:formatNumber value="${arrList[9]}" pattern="#0.0"/>&nbsp;</td>
				
				<td >${arrList[10]}&nbsp;</td>
				<td ><fmt:formatNumber value="${arrList[11]}" pattern="#0.0"/>&nbsp;</td>
				<td>${arrList[12]}&nbsp;</td>
				<td><fmt:formatNumber value="${arrList[13]}" pattern="#0.0"/>&nbsp;</td>
				
				<td >${arrList[14]}&nbsp;</td>
				<td ><fmt:formatNumber value="${arrList[15]}" pattern="#0.0"/>&nbsp;</td>
				<td >${arrList[16]}&nbsp;</td>
				<td ><fmt:formatNumber value="${arrList[17]}" pattern="#0.0"/>&nbsp;</td>
				<td >${arrList[18]}&nbsp;</td>
				<td ><fmt:formatNumber value="${arrList[19]}" pattern="#0.0"/>&nbsp;</td>
				<td >${arrList[20]}&nbsp;</td>
				<td ><fmt:formatNumber value="${arrList[21]}" pattern="#0.0"/>&nbsp;</td>
			</tr>
		</c:forEach>
		<tr style="background-color:yellow;color:red;font-weight:bold;border:1px solid #cccccc">
				<td style="">${strsList[1]}</td>
