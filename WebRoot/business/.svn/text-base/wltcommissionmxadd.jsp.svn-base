...
function check(){
	...
	if(parseInt(_startFee) > parseInt(_endFee)){
  	...
  	with(document.forms[0]){
		action="syscommission.do?method=addmxa";
		target="_self";
		submit();
	}
}
...
<html:form  method="post" action="/business/syscommission.do?method=addmxa">
<div class="pass_main">
    ...
    <div class="pass_list">
        <ul>
            ...
            <li>
   	 		<strong>是否是他省：</strong>
   	 		<span>
   	 			<div class="sub-input"><a class="sia-2 selhover" id="sl-sc_other" href="javascript:void(0);">请选择</a></div>
				<input name="sc_other" id="sc_other" defaultsel="请选择[]|是[0]|否[1]" type="hidden" value="" />
			</span>
  		  	</li>
  		  </ul>
   </div>
