...
function checksubmit(){
	...
	$("#form1").first();
	$("#form1").attr("action","<c:url value='/business/acctbill.do?method=list'/>");
...
<form id="form1" name="form1" action="<%=path %>/business/acctbill.do?method=list" method="post">
<div class="header">
...
<c:if test="${(sessionScope.userSession.roleType == 0) || (sessionScope.userSession.roleType == 1)}">
  <tr>
	 <td>客户经理</td>
    <td><input name="exp1" type="text" value="${requestScope.acctbill.exp1 }"/></td>
		<td>代理商账号</td>	
		<td><input name="dailishang" type="text" value="${requestScope.acctbill.dailishang }"/></td>  
    <td>登录账号：</td>
  ...
  <tr>
	<td>充值号码:</td>
    <td><input name="tradeaccountNumber" type="text" value="${requestScope.acctbill.tradeaccountNumber}"/></td>
	<td>&nbsp;</td>	
	<td>&nbsp;</td>  
    <td>&nbsp;</td>
    <td>&nbsp;</td>
...
<th>交易账号
<!-- &&(sessionScope.userSession.roleType != 0)&&(sessionScope.userSession.roleType != 1)	 -->
<c:if test="${acctbill.tradetype ==11}">
		/名称
	</c:if>
<c:if test="${acctbill.tradetype !=11&&(sessionScope.userSession.roleType == 0||sessionScope.userSession.roleType == 1)}">
		/登录
	</c:if>
</th>
<th>交易时间</th>
...
<div class="grayr">
<wlt:pager url="business/acctbill.do?method=list${requestScope.acctbill.paramUrl}" page="${requestScope.page}" style="black" />
</div>
</td>
</tr>
</tfoot>
