 ...
 * 
 */
public class AuthImg_old extends HttpServlet {

	// 设置图形验证码中字符串的字体和大小
	private Font myFont = new Font("Arial Black", Font.PLAIN, 12);
	...
	private String generateCheckCode(HttpServletRequest request) {
		...
		request.getSession().setAttribute("rand", result + "");
		return checkCode;
	}
		...
		try {
			...
			for (int i = 0; i < baseChar.length; i++) {
				...
				g.setFont(new Font(fontTypes[random.nextInt(fontTypesLength)],
						Font.BOLD, 16+ random.nextInt(6)));
				g.drawString(baseChar[i], 24 * i + 5, 22);
			}

			for (int i = 0; i < (random.nextInt(5) + 10); i++) {
				g.setColor(new Color(random.nextInt(255) + 1, random
						.nextInt(255) + 1, random.nextInt(255) + 1));
				g.drawLine(random.nextInt(150), random.nextInt(50), random
						.nextInt(150), random.nextInt(50));
			}
			g.dispose();
