...
public class QueryReturnServlet extends HttpServlet {
	...
	private static final String Payment = "/bank/TransactionInput.jsp";
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (str.length == 16) {
			...
			int Res_transDate = str[5].indexOf("=");
			int Res_orderNo = str[2].indexOf("=");
			int Res_transAmt = str[3].indexOf("=");
			int Res_curyId = str[4].indexOf("=");
			int Res_transType = str[6].indexOf("=");
			int Res_priv1 = str[8].indexOf("=");
			int Res_transStat = str[7].indexOf("=");
			int Res_openBankId = str[9].indexOf("=");
			...
			String TransDate = str[5].substring(Res_transDate + 1);
			String OrderNo = str[2].substring(Res_orderNo + 1);
			String TransAmt = str[3].substring(Res_transAmt + 1);
			String CuryId = str[4].substring(Res_curyId + 1);
			String TransType = str[6].substring(Res_transType + 1);
			String Priv1 = str[8].substring(Res_priv1 + 1);
			String TransStat = str[7].substring(Res_transStat + 1);
			// transStat表示代扣状态,只有"1001"表示代扣成功，其他状态均表示代扣失败
			String OpenBankId = str[9].substring(Res_openBankId + 1);
			...
			System.out.println("gateId=" + OpenBankId);
			...
			String responseData = responseCode + MerId + TransDate + OrderNo
					...
					+ OpenBankId + CardType + CardNo + UserName + CertType
					+ CertId;
			...
			if (res) {
				...
				request.getRequestDispatcher("./QueryReturn.jsp").forward(
						request, response);
